...
void GxDESP32T_BP::writeScreenBuffer(uint8_t value)
{
...
void GxDESP32T_BP::writeImage(const uint8_t* bitmap, uint32_t size, uint8_t depth, uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
  ...
  uint16_t bm_wb = w; // copy original w here, will be width bytes of bitmap
  if (x + w > _width) w = _width - x;
  if (y + h > _height) h = _height - y;
  uint32_t idx = 0; // source index
  uint32_t n; // words to transfer
  uint16_t ww; // words per row
  uint16_t dw; // destination word
  switch (depth)
  {
    case 0: // corner test
      avt.avt_run_sys();
    ...
    case 1:
      ...
      avt.avt_ld_img_area(EPD_DATA_2BPP, x, y, 8 * bm_wb, h);
      avt.avt_wr_reg_addr(0x0154);
      n = uint32_t(bm_wb) * uint32_t(h);
      avt.startTransfer(n);
    ...
    case 2:
      bm_wb = (bm_wb + 3) / 4;
      ww = (w + 7) / 8; // "must be divisible by 8"
      avt.avt_run_sys();
      avt.avt_wr_reg(0x0020, 0x2);  //big endian
      avt.avt_ld_img_area(EPD_DATA_2BPP, x, y, 8 * ww, h);
      avt.avt_wr_reg_addr(0x0154);
      n = uint32_t(ww) * uint32_t(h);
      avt.startTransfer(n);
      for (uint16_t r = 0; r < h; r++)
      {
        for (uint16_t cw = 0; cw < ww; cw++)
        {
          ...
          else dw = 0xFF | (idx < size ? bitmap[idx++] : 0xFF) << 8;
          avt.transfer16(dw);
        }
      }
      avt.endTransfer();
    ...
    case 4:
      bm_wb = (bm_wb + 1) / 2;
      ww = (w + 3) / 4; // "must be divisible by 4"
      avt.avt_run_sys();
      avt.avt_wr_reg(0x0020, 0x2);  //big endian
      avt.avt_ld_img_area(EPD_DATA_4BPP, x, y, 4 * ww, h);
      avt.avt_wr_reg_addr(0x0154);
      n = ww * uint32_t(h);
      avt.startTransfer(n);
      for (uint16_t r = 0; r < h; r++)
      ...
      break;
    case 8:
      ww = (w + 1) / 2; // "must be divisible by 2"
      avt.avt_run_sys();
      avt.avt_wr_reg(0x0020, 0x2);  //big endian
      avt.avt_ld_img_area(EPD_DATA_8BPP, x, y, 2 * ww, h);
      avt.avt_wr_reg_addr(0x0154);
      n = uint32_t(ww) * uint32_t(h);
      avt.startTransfer(n);
      for (uint16_t r = 0; r < h; r++)
...
void GxDESP32T_BP::writeImagePart(const uint8_t* bitmap, uint32_t size, uint8_t depth, int16_t x_part, int16_t y_part, int16_t w_bitmap, int16_t h_bitmap,
...
void GxDESP32T_BP::drawImagePart(const uint8_t* bitmap, uint32_t size, uint8_t depth, int16_t x_part, int16_t y_part, int16_t w_bitmap, int16_t h_bitmap,
                                 int16_t x, int16_t y, int16_t w, int16_t h)
{
